...
CREATE TABLE users (
  ...
  notification_preferences JSONB DEFAULT '{dailyReminders: true, progressReminders: true, weeklyReview: true, deadlineAlerts: true, teamNotifications: true, reminderTime: 09:00}'::jsonb,
...
CREATE TABLE projects (
  ...
  description TEXT,
  area VARCHAR(50) DEFAULT 'general' CHECK (area IN ('marketing', 'product', 'operations', 'general')),
  plan_format VARCHAR(50) DEFAULT 'standard' CHECK (plan_format IN ('lean', 'standard', 'detailed')),
...
**area** controla la plantilla de plan a utilizar (`marketing`, `product`, `operations`, `general`).
**plan_format** define el nivel de detalle (`lean`, `standard`, `detailed`).
...
CREATE INDEX idx_plan_iterations_project_id ON plan_iterations(project_id);
CREATE INDEX idx_plan_iterations_user_id ON plan_iterations(user_id);
```

Conserva cada reintento del plan generado por IA junto con el feedback del usuario.
